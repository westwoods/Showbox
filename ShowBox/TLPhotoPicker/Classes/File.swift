...
//		
//		@discardableResult
//		func getAllPhotos( result: inout [TLAssetsCollection]) -> TLAssetsCollection? {
//			
//			let fetchOptions = PHFetchOptions()
//			
//			let sortOrder = [NSSortDescriptor(key: "creationDate", ascending: false), ]
//			fetchOptions.sortDescriptors = sortOrder
//			let fetchCollection = PHAsset.fetchAssets(with: (.image ) , options: fetchOptions)
//			//	let fetchCollectionv = PHAsset.fetchAssets(with: (.video ) , options: fetchOptions)
//			//	if let collection = fetchCollection.firstObject, !result.contains(where: { $0.localIdentifier == collection.localIdentifier }) {
//			
//			let assetsCollection = TLAssetsCollection()
//			assetsCollection.fetchResult = fetchCollection
//			if assetsCollection.count > 0 {
//				assetsCollection.endDate = assetsCollection.getAsset(at: 0)?.creationDate
//				assetsCollection.startDate = assetsCollection.getAsset(at: assetsCollection.count-1)?.creationDate
//				
...
//			//Camera Roll
//			let camerarollCollection = getAllPhotos(result: &assetCollections)
